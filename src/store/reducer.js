...
const dataFromCsvLoaded = (state, action) => {
  ...
  const data = action.data.map(
    row => (row = { id: Math.random().toFixed(5), ...row })
  );
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case actionTypes.GET_DATA_FROM_CSV:
      return getDataFromCsv(state, action);
    case actionTypes.DATA_FROM_CSV_LOADED:
      return dataFromCsvLoaded(state, action);
    case actionTypes.GET_DATA_FROM_CSV_FAILED:
      return getDataFromCsvFailed(state, action);

    case actionTypes.UPDATE_TABLE_DATA:
      return updateData(state, action);
    case actionTypes.UPDATE_TABLE_TITLE:
